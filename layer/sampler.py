...
#ypergraph sampler with two level hypergraph for next-poi task
class Neighbor(torch.utils.data.DataLoader):
    """
    Args:
        x: feature matrix.
    ...
    def __init__(
        ...
        super(Neighbor, self).__init__(sample_idx.view(-1).tolist(), collate_fn=self.sample, **kwargs)
    ...
    def sample(self, batch):
        ...
        for i, size in enumerate(self.sizes):
            ...
            if adj_t.nnz():
                assert size[0] >= col.max() + 1, '[Neighbor] adj_t source max index exceed sparse_sizes[1]'
            else:
                #empty subgraph
        ...
        if row[target_mask].size(0) == 0:
            raise ValueError(
                f'[Neighbor] All trajectories have no checkin before target time!!'
            )
        adj_t = SparseTensor(
           row=row[target_mask],
           col=col[target_mask],
        ...
        if adj_t.storage.row().unique().shape[0] != batch_size:
            ...
            raise ValueError(
                f'[Neighbor] Trajectory without checkin neighbors after filtering by max_time is not allowed!!\n'
        ...
        return out
    #The original traj2traj topology is in adj_t, we set the value to all ones, and then we substitute it with traj2traj_jaccard, and keep the data within [0, 1]
    def filter_traj2traj_with_leakage(self, adj, traj_size, mode=1):
        ...
        if mode == 1:
            ...
            if adj_t.nnz() != he2he.nnz():
                raise ValueError(f"[Neighbor] he2he filtered size not equal.")
