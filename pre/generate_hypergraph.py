...
from util.math import haversine
import os
import os.path as osp
import logging


def generate_hypergf(input_file, output_path, args):
    """
    ...
    #if true, shift traj_id with offset #check_ins_id before saving to pyg data, which means idx of checkin are in range [0, #checkin_id - 1], and idx of trajectory are in range [#checkin, #trajectory+#checkin-1]
    traj_offset = True
    if traj_offset:
        checkin_offset = torch.as_tensor([data.check_ins_id.max() + 1], dtype=torch.int64)
    else:
        checkin_offset = torch.as_tensor([0], dtype=torch.int64)

    traj_stat = generate_hyperedg(data, traj_column)
    ci2traj_pyg_data = generate_ci2traj(data, traj_stat, traj_column, checkin_offset)

    traj2traj_intra_u_data = generate_traj2traj(
        data,
        traj_stat,
        traj_column,
        threshold,
    ...
    traj2traj_inter_u_data = generate_traj2traj(
        data,
        traj_stat,
        traj_column,
        threshold,
    ...
    traj2traj_pyg_data = merge_traj2traj(traj_stat, traj2traj_intra_u_data, traj2traj_inter_u_data, checkin_offset)
...
def generate_hyperedg(data, traj_column):
    """
...
def generate_ci2traj(data, traj_stat, traj_column, checkin_offset):
    """
...
def generate_traj2traj(
    ...
    #First create sparse matrix for trajectory -> poi, then generate inter-user adjacency list one trajectory may have multiple identical poi_id, we drop the duplicate ones first
    ...
    if relation_type == 'inter':
        row_filtered, col_filtered, data_filtered = filter_hyperedge_connections(
            row=traj2traj.row,
            col=traj2traj.col,
            data=traj2traj.data,
            chunk_num=chunk_num,
    ...
    #filter based on pre-define conditions
    ...
    if relation_type == 'inter':
        #filter based on pre-define metric threshold
        row_filtered, col_filtered, data_filtered = filter_hyperedge_connections(
            row=traj2traj.row,
            col=traj2traj.col,
            data=traj2traj.data,
...
def merge_traj2traj(traj_stat, intra_u_data, inter_u_data, checkin_offset):
    """ 
    :param traj_stat: hyperedge(trajectory) statistics;
...
def filter_hyperedge_connections(row, col, data, he_size, chunk_num=10, threshold=0.02, filter_mode='min size'):
    """
