...
from pre.pre_funcs import encodeID, dropfirst, keeplast
...
class GenerateBase:
    ...
    @classmethod
    def read_data(cls, file_name, dataset_name):
        raise NotImplementedError


class GenerateFile(GenerateBase):
    @classmethod
    def read_data(cls, file_name, dataset_name):
    ...
    @classmethod
    def filter_low_freq(cls, df, poi_min_freq, user_min_freq):
    ...
    @classmethod
    def split_data(cls, df, is_sorted=False):
        if not is_sorted:
        ...
        df.iloc[validation_index:test_index, df.columns.get_loc('SplitTag')] = 'validation'
        df.iloc[test_index:, df.columns.get_loc('SplitTag')] = 'test'
        df['UserRank'] = df.groupby('UserId')['UTCTimeOffset'].rank(method='first')
    ...
    @classmethod
    def generateID(cls, df, session_time_interval, do_label_encode=True, only_last_metric=True):
        df = df.sort_values(by=['UserId', 'UTCTimeOffset'], ascending=True)
        ...
        if do_label_encode:
            ...
            poi_id_le, padding_poi_ie = encodeID(df_train, df, 'PoiId', padding=0)
            poi_category_le, padding_poi_category = encodeID(df_train, df, 'PoiCategoryId', padding=0)
            user_id_le, padding_user_id = encodeID(df_train, df, 'UserId', padding=0)
            hour_id_le, padding_hour_id = encodeID(df_train, df, 'UTCTimeOffsetHour', padding=0)
            weekday_id_le, padding_weekday_id = encodeID(df_train, df, 'UTCTimeOffsetWeekday', padding=0)
