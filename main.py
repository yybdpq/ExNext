...
import random
import argparse
from exnext import EXNEXT
from torch import nn
from tqdm import tqdm
from torch.utils.tensorboard import SummaryWriter
from pre.pre_main import preprocess
from util.funcs import seedset, logset
from util.cfg import Cfg
from util.funcs import count_parameters, test_step, save_model
from layer.sampler import Neighbor
from pre.dataset import LBSNDataset
...
if __name__ == '__main__':
    ...
    if cfg.model_args.model_name == 'exnext':
        model = EXNEXT(cfg)
    else:
        raise NotImplementedError(
            f'[Training] Model {cfg.model_args.name}, please choose from ["exnext"]'
        )
    ...
    if cfg.run_args.do_train:
        ...
        for eph in range(cfg.run_args.epoch):
            ...
            for data in tqdm(sampler_train):
                ...
                label=data.y[:, 0]
                #get adj_mask_prob and adj_mask_matrix
                _, adj_mask_prob, adj_mask_matrix= model(input_data, label)
                out, _, _= model(input_data, label, adj_mask_matrix)

                AIB_prior = (0.8 * torch.ones_like(adj_mask_prob)).to(device=model.device)
                ...
                aib_kl_loss = 0.01 * aib_kl_loss.sum()
